...
<script src="<%=resourceUrl %>/script/admin/action_inc.js?v=2014050901" type="text/javascript" charset="UTF-8"></script>
    ...
    <c:if test="${ !(param.tag_text eq 'hide') }">
       		...
       		<div style="width: 100%;">
	       		...
	       		<div style="float: left;">
			    	...
			    	<div style="height:35px; width:410px; float: left;margin-top: 270px; ">
				    	 ...
				    	 <span class="btn btn_input btn_primary">
				    	 	<button onclick="submitMsgActionForm('news');">${ not empty param.btn_submit_text ? param.btn_submit_text : "保存"}</button>
						 </span>
	       			</div>
	       		</div>
	       		<%--<div class="clear"></div>
    ...
    <c:if test="${ !(param.tag_exp eq 'hide') }">
	    		...
	    		<div id="txt_btn" style="float: left;">
	    			<c:if test="${ !('hide' eq param.btn_return) }">
			    	 ...
			    	 <span id="js_submit" class="btn btn_input btn_primary">
			    	 	<button onclick="submitMsgActionForm('api');">${ not empty param.btn_submit_text ? param.btn_submit_text : "保存"}</button>
					 </span>
			    </div>
		    </div>
        </div>

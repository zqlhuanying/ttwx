...
public class SearchFundService implements MenuExtService {
    ...
    public String extProcess(){
        ...
        // 若用户手机号不存在，则视为未绑定
        if( !userFundService.isBindAccount(fromUserName) ){
            respTextMessage.setContent("发现您未绑定手机号码，<a href=\"" + AppConfig.DOMAIN_PAGE + "/admin/extmenu/bindAccount?id=" + fromUserName + "\">点击这里，进行绑定</a>");
        }else {
